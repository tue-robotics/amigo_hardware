...

{
  ...
  addProperty( "direction", direction).doc("double -1 or 1 ");
...
bool CalculateFFW::configureHook()
{
  ...
  if ( direction != -1.0 && direction != 1.0 ) {
    log(Warning)<<"CalculateFFW: Could not configure component. Direction should be either -1.0 or 1.0!"<<endlog();
    return false;
  }
...
void CalculateFFW::updateHook()
{
  ...
  // Calculating FFW
  FFW[0] = direction*(FFWgrav + FFWstat*sign(ref_vel[0]) + FFWdyn*ref_vel[0] + FFWacc*ref_acc[0]);
