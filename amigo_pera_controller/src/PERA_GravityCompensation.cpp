...
using namespace PERA;
	...
	GRAVCOM::GRAVCOM(const std::string& name)
...

		{
			...
			addProperty( "torqueFactors", gCTF );
	...
	void GRAVCOM::updateHook(){
		...
		/// Gravity compensation
		q << -jointAngles[0]*(PI/180), -jointAngles[1]*(PI/180)-(PI/2), jointAngles[2]*(PI/180), jointAngles[3]*(PI/180), jointAngles[4]*(PI/180), jointAngles[5]*(PI/180)-(PI/2), jointAngles[6]*(PI/180);

		gravComp = ComputeGravity(a,d,alpha,coglist,mlist,Istore,q,grav);

		additionalTorques[0]=gravComp(0,0)*gCTF[0];
		additionalTorques[1]=gravComp(1,0)*gCTF[1];
		additionalTorques[2]=gravComp(2,0)*gCTF[2];
		additionalTorques[3]=gravComp(3,0)*gCTF[3];
		additionalTorques[4]=gravComp(4,0)*gCTF[4];
		additionalTorques[5]=gravComp(5,0)*gCTF[5];
		additionalTorques[6]=gravComp(6,0)*gCTF[6];
		
		gravCompPort.write(additionalTorques);

	}

	void GRAVCOM::stopHook(){


	}
	
	// Matrix computing the rotation matrix from link i-1 to link i, using the DH-parameters
	Eigen::Matrix3d GRAVCOM::ComputeRotationMatrix(double d,double alpha,double q){
		Eigen::Matrix3d R;
	...
	Eigen::Matrix3d GRAVCOM::eul2rot(double phi, double theta, double psi){
		Eigen::Matrix3d R;
		R = Eigen::Matrix3d::Zero(3,3);
	...
	Eigen::MatrixXd GRAVCOM::rne(Eigen::MatrixXd a,Eigen::MatrixXd d,Eigen::MatrixXd alpha,Eigen::MatrixXd coglist,Eigen::MatrixXd mlist,Eigen::MatrixXd Istore,Eigen::MatrixXd Q,Eigen::MatrixXd Qd,Eigen::MatrixXd Qdd,Eigen::MatrixXd grav, Eigen::MatrixXd Fext){
	...
	Eigen::MatrixXd GRAVCOM::ComputeGravity(Eigen::MatrixXd a,Eigen::MatrixXd d,Eigen::MatrixXd alpha,Eigen::MatrixXd coglist,Eigen::MatrixXd mlist,Eigen::MatrixXd Istore,Eigen::MatrixXd q,Eigen::MatrixXd grav){
...
ORO_CREATE_COMPONENT(PERA::GRAVCOM)
